...
def display_img(column, path, final_data, name, available_colors):
    ...
    with column:
        with st.container():
            ...
            action = st.pills(
                "", ["Flag", "Relabel", "Remove"], key=path + name, selection_mode="single"
            )
            ...
            if action == "Remove" and "dataset_name" in st.session_state:
                ...
                new_desc = (
                    ...
                    if not current_desc.startswith("[FLAGGED]")
                    else current_desc
                )
                api_client.update_annotation(
                    st.session_state["dataset_name"], path, current_label, new_desc
                )
                st.warning("Flagged!")
                ...
                with st.popover("✏️ Edit Annotation", use_container_width=True):
                    ...
                    current_index = (
                        available_label_list.index(current_label)
                        if current_label in available_label_list
                        else 0
                    )
                    ...
                    with col1:
                        # AI Analyze button
                        if st.button(
                            "🤖 AI Analyze", key=f"analyze_{path}_{name}", use_container_width=True
                        ):
                            with st.spinner("Analyzing with AI..."):
                                ...
                                if result.get("success"):
                                    ...
                                    cached_data = api_client.get_annotations(
                                        st.session_state["dataset_name"]
                                    )
                                    if cached_data.get("total_annotations", 0) > 0:
                                        ...
                                        for idx, row in st.session_state[
                                            "final_data_df"
                                        ].iterrows():
                                            if row["path"] in path_to_annotation:
                                                cached = path_to_annotation[row["path"]]
                                                st.session_state["final_data_df"].at[
                                                    idx, "label"
                                                ] = cached.get("label", "pending")
                                                st.session_state["final_data_df"].at[
                                                    idx, "description"
                                                ] = cached.get("description", "No description")
                                                st.session_state["final_data_df"].at[
                                                    idx, "patient"
                                                ] = str(cached.get("patient_id", "anonymous"))
                    ...
                    with col2:
                        # Manual Save button
                        if st.button(
                            "💾 Save Manual", key=f"save_{path}_{name}", use_container_width=True
                        ):
                            with st.spinner("Saving..."):
                                result = api_client.update_annotation(
                                    st.session_state["dataset_name"], path, new_label, new_desc
                                )
                                if result.get("success"):
                                    # Update local dataframe
                                    st.session_state["final_data_df"].loc[
                                        st.session_state["final_data_df"]["path"] == path, "label"
                                    ] = new_label
                                    st.session_state["final_data_df"].loc[
                                        st.session_state["final_data_df"]["path"] == path,
                                        "description",
                                    ] = new_desc
            ...
            st.markdown(
                f"<span style='background-color:{label_color};padding:4px 8px;border-radius:4px;margin:2px'>{current_label}</span> | Patient: ``{current_patient}``",
